  ...
  return async function(req, res) {
    ...
    if (corsAnywhere.originByCountryBlacklist.length && corsAnywhere.originByCountryBlacklist.indexOf(location.host) > -1) {
      ...
      if (corsAnywhere.countryBlacklist.length) {
        let ip = (req.headers && req.headers['x-forwarded-for']) || req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
        console.log('ip:', ip);
        let locationInfo;
        ...
        console.log('Country Code:', locationInfo && locationInfo.countryCode);
