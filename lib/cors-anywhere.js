...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originByCountryBlacklist: [],   // If non-empty, requests not from an origin in this list and country code from IP in "countryBlacklist" list will be blocked.
    countryBlacklist: [],           // If non-empty, requests not from country code(is populated from IP) in this list will be blocked.
  ...
  return async function(req, res) {
    ...
    // console.log('corsAnywhere.originByCountryBlacklist:', corsAnywhere.originByCountryBlacklist);
    // console.log('corsAnywhere.countryBlacklist:', corsAnywhere.countryBlacklist);
    if (corsAnywhere.originByCountryBlacklist.length && corsAnywhere.originByCountryBlacklist.indexOf(location.host) > -1) {
      if (corsAnywhere.countryBlacklist.length) {
        let ip = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
        console.log('ip:', ip);
        let locationInfo;

        try {
          locationInfo = await iplocation(ip);
        } catch (err) {
          console.log('Can not get location from IP');
        }

        console.log('locationInfo:', locationInfo && locationInfo.countryCode);

        if (locationInfo && locationInfo.countryCode && corsAnywhere.countryBlacklist.indexOf(locationInfo.countryCode.toUpperCase()) > -1) {
          res.end();
          return;
        }
      }
